...
document.addEventListener('DOMContentLoaded', () => {
        ...
        async function generateRoster() {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    // Backend endpoint yoksa, mock data kullan
                    if (!responseData || Object.keys(responseData).length === 0 || !responseData.pilots || responseData.pilots.length === 0) {
                        const selectedFlight = JSON.parse(sessionStorage.getItem('selectedFlight') || '{}');
                        responseData = {
                            flightNumber: selectedFlight.flight_no || 'N/A',
                            aircraft: selectedFlight.vehicle_type || 'N/A',
                            date: selectedFlight.date_time || new Date().toISOString(),
                            pilots: [
                                { id: 1, name: 'Captain John Smith', certType: 'CPT' },
                                { id: 2, name: 'First Officer Jane Doe', certType: 'FO' }
                            ],
                            cabinCrew: [
                                { id: 3, name: 'Alice Johnson', type: 'FA' },
                                { id: 4, name: 'Bob Wilson', type: 'FA' },
                                { id: 5, name: 'Carol Davis', type: 'Purser' }
                            ],
                            passengers: [
                                { id: 101, name: 'Passenger 1', assignedSeat: null },
                                { id: 102, name: 'Passenger 2', assignedSeat: null },
                                { id: 103, name: 'Passenger 3', assignedSeat: null }
                            ],
                            aircraftCapacity: 180,
                            rules: [
                                { code: 'DR-01', name: 'Pilot Composition (DR-01)', passed: true },
                                { code: 'DR-03', name: 'Cabin Composition (DR-03)', passed: true },
                                { code: 'DR-02', name: 'Vehicle/Distance Limits (DR-02)', passed: true },
                                { code: 'DR-04', name: 'Capacity & Seats (DR-04)', passed: true }
                            ]
                        };
                    }
    ...
    function initializeSeatAssignment() {
        ...
        function renderSeatGrid() {
            ...
            for (let i = 0; i < numSeats; i++) {
                ...
                const seatNumber = String.fromCharCode(65 + Math.floor(i / seatsPerRow)) + (i % seatsPerRow + 1);
        ...
        autoAssignBtn.addEventListener('click', async () => {
            ...
            try {
                // Yolcuları koltuplara otomatik ata (mock logic)
                let seatIndex = 0;
                const passengers = currentRosterDraftS4.passengers || [];
                
                passengers.forEach((passenger, idx) => {
                    if (!passenger.assignedSeat && seatIndex < (currentRosterDraftS4.aircraftCapacity || 100)) {
                        const seatsPerRow = 6;
    ...
    let currentRosterDraftS5_local = null; 
    ...
    function initializeRosterEdit() {
        ...
        approvalBtn.addEventListener('click', async () => {
            ...
            try {
                // Backend approval endpoint yoksa, mock approval yap
                if (currentRosterDraftS5) {
                    const approvedRoster = {
                        ...currentRosterDraftS5,
                        status: 'APPROVED',
    ...
    function initializeFinalManifest() {
        ...
        function renderFinalManifest(roster) {
            ...
            if (pilotList) {
                ...
                (roster.pilots || []).forEach(pilot => {
                    const li = document.createElement('li');
                    li.innerHTML = `<strong>${pilot.name}</strong> - ${pilot.certType || 'Captain'}`;
                    pilotList.appendChild(li);
                });
            }
            ...
            if (cabinList) {
                ...
                (roster.cabinCrew || []).forEach(crew => {
                    const li = document.createElement('li');
                    li.innerHTML = `<strong>${crew.name}</strong> - ${crew.type || 'Flight Attendant'} (${crew.language || 'EN'})`;
                    cabinList.appendChild(li);
                });
            }
